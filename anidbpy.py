...
if __name__ == "__main__":
    async def test():
        ...
        ep_num = 1
        print(f"Testing AniDB.app extraction for AniList ID {anilist_id}, Episode {ep_num}...")
        try:
            ep_data = await get_episodes(anilist_id)
            print("Episodes count (SUB):", len(ep_data["episodes"].get("sub", [])))
            print("\nMeta:", json.dumps(ep_data["meta"], indent=2))

            watch_data = await handle_watch(anilist_id, "sub", ep_num)
            print("\nWatch streams result:", json.dumps(watch_data, indent=2))
